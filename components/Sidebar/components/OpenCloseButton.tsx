    ...
    <>
      <button
        className={`fixed top-10 ${
          ...
          side === 'right' ? 'right-[270px]' : 'left-[270px]'
        } sm:h-11 sm:w-8 sm:text-neutral-700`}
        onClick={onClick}
      >
...
export const OpenSidebarButton = ({ onClick, side }: Props) => {
  return (
    <button
      ...
      } z-50 h-10 w-7 text-white hover:text-gray-400 dark:text-white dark:hover:text-gray-300 sm:top-0.5 sm:${
        side === 'right' ? 'right-2' : 'left-2'
      } sm:h-10 sm:w-8 sm:text-neutral-700`}
      onClick={onClick}
    >
